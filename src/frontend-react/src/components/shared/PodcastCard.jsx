...
export default function PodcastCard({ podcast }) {
    ...
    const togglePlayPause = async (episodeId) => {
        ...
        if (currentlyPlaying === episodeId) {
            ...
            audio.src = DataService.GetPodcastAudio(episodeId + "-EN.mp3")+ `?v=${Date.now()}`;
    ...
    return (
        <div className={styles.episodeCard}>
            ...
            <Image
                src={`/assets/${podcast.image}`}
                alt={podcast.title}
                width={600}
                height={500}
                className={styles.birdImage}
            />
            <p className={styles.caption}>{podcast.caption}</p>
            </div>

            <div className={styles.episodeHeader}>
                <div className={styles.episodeInfo}>
                    <span className={styles.podcast}>Bird Calls</span>
