...
def download_from_gcs(INPUT_FOLDER, bucket_name):
    # Clear and recreate the local folder
    ...
    for blob in blobs:
        ...
        if blob.name.endswith(".txt"):  
            local_path = os.path.join(INPUT_FOLDER, os.path.relpath(blob.name, INPUT_FOLDER))
            os.makedirs(os.path.dirname(local_path), exist_ok=True)
            # Download the blob to the local file
            blob.download_to_filename(local_path)
            downloaded_files.append(os.path.basename(local_path))
            print(f"Downloaded to: {local_path}")
    return downloaded_files

downloaded_files = download_from_gcs(INPUT_FOLDER, bucket_name)
...
- If asked about topics unrelated to birds, politely redirect the conversation back to bird-related subjects.
- Be concise in your responses while ensuring you cover all relevant information from the chunks.

Your goal is to provide accurate, helpful information about birds based solely on the content of the text chunks you receive with each query.
"""
...
def load_text_embeddings(df, collection, batch_size=500):
	...
	hashed_books = df["species"].apply(lambda x: hashlib.sha256(x.encode()).hexdigest()[:16])
...
def embed(method="char-split"):
	...
	# Process
	for jsonl_file in jsonl_files[:6]:
		...
		# text_files = glob.glob(os.path.join(data_df, "embeddings-*.txt"))
		text_files = glob.glob(os.path.join(OUTPUT_FOLDER, "embeddings-*.jsonl"))
		print(f"Text files to upload: {text_files}")

		for text_file in text_files:
			filename = os.path.basename(text_file)
			destination_blob_name = os.path.join(OUTPUT_FOLDER, filename)
...
def query(method="char-split"):
	...
	query = "Where can Andigena hypoglauca be found?"
	...
	search_string = "Andigena hypoglauca"
	results = collection.query(
		query_embeddings=[query_embedding],
		n_results=10,
...
def agent(method="char-split"):
	...
	user_prompt_content = Content(
    	role="user",
		parts=[
			Part.from_text("Describe the migration path of Andigena hypoglauca and its habitat."),
		],
	)
...
def download():
    ...
    blobs = bucket.list_blobs(prefix=f"{OUTPUT_FOLDER}/")
    
    for blob in blobs:
        file_path = os.path.relpath(blob.name, OUTPUT_FOLDER)
        lfile_path = os.path.join(OUTPUT_FOLDER, file_path)
        
        os.makedirs(os.path.dirname(lfile_path), exist_ok=True)
        # print(f"Downloading embeddings {blob.name} to {lfile_path}")
        blob.download_to_filename(lfile_path)
        print(f"Downloaded embeddings {blob.name} to {lfile_path}")
...
def main(args=None):
	...
	if args.audio_file:
		chat(method=args.chunk_type, audio_file=args.audio_file)
