...
async def start_chat_with_llm(message: Dict, x_session_id: str = Header(None, alias="X-Session-ID")):
    ...
    if title == "":
        title =  "Audio chat" #updated from Image to Audio
    title = title[:50] + "..."
    chat_response = {
        "chat_id": chat_id,
        "title": title,
...
@router.get("/audio/{chat_id}/{message_id}.mp3") #updated from png to mp3/images to audio
async def get_chat_audio(chat_id: str, message_id: str): #updated fuc name
    """
    Serve an audio from the chat history.
    ...
    try:
        ...
        audio_path = os.path.join(
            chat_manager.audio_dir,
            chat_id,
            f"{message_id}.mp3" #updated from png to mp3
        )
        
        # Verify the path exists and is within the audio directory
        audio_path = Path(audio_path).resolve()
        audio_dir = Path(chat_manager.audio_dir).resolve()
        
        # Security check: ensure the requested file is within the audio directory
