...
class ChatHistoryManager:
    def __init__(self, model, history_dir: str = "chat-history"):
        ...
        self.audio_dir = os.path.join(self.history_dir, "audios")
    ...
    def _save_audio(self, chat_id: str, message_id: str, audio_data: str) -> str:
        """
        Save audio data to a file and return the relative path.
        ...
        Returns:
            str: Relative path to the saved audio
        """
        # Create chat-specific audio directory
        chat_audio_dir = os.path.join(self.audio_dir, chat_id)
        os.makedirs(chat_audio_dir, exist_ok=True)
        
        # Save audio to file
        audio_path = os.path.join(chat_audio_dir, f"{message_id}.mp3")
        try:
            # Extract the actual base64 data and mime type
            base64_string = audio_data
            if ',' in base64_string:
                header, base64_data = base64_string.split(',', 1)
                mime_type = header.split(':')[1].split(';')[0]
            else:
                base64_data = base64_string
                mime_type = 'audio/mp3'  # default to JPEG if no header
            ...
            return os.path.relpath(audio_path, self.history_dir)
        except Exception as e:
            print(f"Error saving audio: {str(e)}")
            traceback.print_exc()
            return ""

    def _load_audio(self, relative_path: str) -> Optional[str]:
        """
        Load audio data from file and return as base64.
        
        Args:
            relative_path: Relative path to the audio from chat history root
        
        Returns:
            Optional[str]: Base64 encoded audio data or None if loading fails
        """
