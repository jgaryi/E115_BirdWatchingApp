...
# Preprocess audio (pad/trim to 3s)
def preprocess_audio_bytes_to_wav_segment(audio_bytes):
    ...
    # Pad or trim to exactly 3 seconds (3000 ms)
...
def analyze_birdnet_from_audio_segment(audio_segment, confidence_threshold=0.5):
    wav_io = BytesIO()
...
def detect_species_from_audio(audio_bytes):
    ...
    if birdnet_result is None or birdnet_result.get("average_confidence", 0) < 0.5:
        ...
        return {
            ...
            "source": "own custom model for local species"  # Ensure 'source' key is always present
        }
    ...
    return {
        ...
        "source": "birdnet"  # Ensure 'source' key is always present
    }
