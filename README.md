...
-   Audio-Based Species Detection: Accepts bird audio recordings and processes them through the BirdNET model to identify the bird species.
-   Text Query Handling: Supports natural language inputs from users and sends them to the LLM agent for contextual answers.
-   Unified Response API: Wraps BirdNET predictions with informative responses generated by the LLM agent, returning a rich description to the user.
-   Metadata & Diagnostics: Provides endpoints for checking model status, version info, and overall API health.
